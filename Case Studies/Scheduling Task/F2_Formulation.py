...
def job_sequence_constraint(m, i, j):
    if pyo.value(i) != pyo.value(j):
        return m.y[i, j] + m.y[j, i] == 1
    else:
        return pyo.Constraint.Feasible


def completion_time_constraint(m, j):
    return m.C[j] == m.S[j] + m.p[j]


def start_time_constraint_1(m, i, j):
    return m.S[i] >= m.C[j] - bigM * m.y[i, j]


def start_time_constraint_2(m, i, j):
    return m.S[j] >= m.C[i] - bigM * m.y[j, i]
...
def get_schedule(m):
    ...
    for i in m.J:
        jobs_before = -1
        for j in m.J:
            if pyo.value(m.y[i, j]) > 1 - f.eps:
                jobs_before += 1
        temp_dct[i] = jobs_before

    schedule = sorted(temp_dct.keys(), key=lambda x: temp_dct[x], reverse=True)
    print(schedule)
...
def main(relaxed=False, time_limit=10, focus=0, termination_gap=0, counter_limit=5, tolerance=1):
    ...
    m = pyo.ConcreteModel(name="F2 Scheduling Model")
    m.I = pyo.Set(initialize=range(1, len(jobs) + 1), doc="Index range of jobs, j", name="Set of Jobs")
    m.J = pyo.Set(initialize=range(1, len(jobs) + 1), doc="Index range of jobs, j", name="Set of Jobs")

    m.y = pyo.Var(m.I, m.J, within=pyo.Binary if not relaxed else pyo.NonNegativeReals, bounds=[0, 1],
                  name="Y(i, j)")
    m.C = pyo.Var(m.J, within=pyo.NonNegativeReals, name="C(j)")
    m.S = pyo.Var(m.J, within=pyo.NonNegativeReals, name="S(j)")
    ...
    m.C1 = pyo.Constraint(m.I, m.J, rule=job_sequence_constraint)
    m.C2 = pyo.Constraint(m.J, rule=completion_time_constraint)
    m.C3 = pyo.Constraint(m.I, m.J, rule=start_time_constraint_1)
    m.C4 = pyo.Constraint(m.I, m.J, rule=start_time_constraint_2)
